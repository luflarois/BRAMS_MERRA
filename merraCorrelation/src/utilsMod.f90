...
module utilsMod
   ...
   recursive function isValid(isp,jsp,nx,ny,valid,aerodata,sName) result(result)

      integer, intent(in) :: isp,jsp,nx,ny
      real,intent(in) :: aerodata(nx,ny)
      logical :: result(nx,ny)
      logical,intent(in) :: valid(nx,ny)
      integer :: xlim(ny,2),ylim(nx,2)
      character(len=*) :: sName


      integer :: i,j

      print *,isp,jsp,trim(sName)

      if(isp==0 .or. jsp==0) return
      result(isp,jsp)=.true.
      do i=isp+1,nx
         xlim(jsp,2)=i
         result(i,jsp)=.true.
         if(aerodata(i,jsp)<0.5) then
            exit
          endif
      enddo
      do i=isp-1,1,-1
      ...
      !print *,xlim(jsp,1),xlim(jsp,2)
      if(xlim(jsp,1)==0 .or. xlim(jsp,2)==0) return


      do i=xlim(jsp,1),xlim(jsp,2)
         do j=jsp+1,ny
            ylim(i,2)=j
            result(i,j)=.true.
            if(aerodata(i,j)<0.5) then
               exit
            endif
         enddo
         do j=jsp-1,1,-1
         ...
         enddo   
         result=isValid(i,j,nx,ny,valid,aerodata,sName)
      enddo  

      !do j=ylim(jsp,1),ylim(jsp,2)
      !  do i=xlim(isp,1),xlim(isp,2)
